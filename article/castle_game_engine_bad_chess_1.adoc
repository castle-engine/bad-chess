...
Now that the proper chessboard with chess pieces is designed, let's add some physics to make things _crazier_.

_Castle Game Engine_ has a support for _rigid body physics_. This means that:

- Objects can be affected by _forces_.
+
The force that works automatically is _gravity_, pulling objects down (in the negative Y direction, by default).
+
You can also define additional forces from code, to e.g. push things along given direction. Your own forces can realize a range of real-life effects, like wind, explosions, spinning tornadoes, etc.
...
Any component descending `TCastleTransform`, including primitives (like `TCastleBox`) or scenes loaded from models (`TCastleScene`) or a group of other objects (`TCastleTransform` with children) can be a _rigid body_ for the physics engine that participates in the collision detection and resulting movement. To do this, you need to add two _behaviors_ to the given component:
...
2. Any component descending from abstract `TCastleCollider`. Multiple shapes are available, like `TCastleSphereCollider`, `TCastleBoxCollider` and `TCastleMeshCollider`.
+
Using the `TCastleMeshCollider` results in most precise collisions, but the colliding object must be _static_ which means that other objects will _bounce off_ this object, but it will not move itself.
...
Next choose any chess piece (I chose one white pawn). Right-click on it and from the context menu choose _"Add Behavior (Extends Parent Transform) -> Physics -> Collider -> Box"_. Note that we use a simpler collider for the chess piece, which is also dynamic. This will allow the chess piece to actually fall down on the board.
...
image::physics_above.png[Chess piece (white pawn) with physics above the board]
...
Make sure that the initial position of all rigid bodies does not make some pair collide with each other right at the start -- since this is impossible in reality. If the 2 objects will collide at start, physics engine may (sometimes quite explosively) move them away from each other. This is not a problem, but it may be surprising at first.

This is a sample result:

image::images/physics_chess_more_dramatic.png[Chess pieces after a more dramatic physics simulation]

One last thing remains to learn in this article part: how to _flick_ the chess piece?

1. From Pascal code you can use various methods to apply a forces on a rigid body. More about this in the next article part. You can also experiment with the example application `examples/physics/physics_forces/` in you're impatient.

2. You can set a specific `LinearVelocity` on a rigid body component.

We will use the latter approach, as it can be trivially done and tested in the editor.

Select the chess piece. Any chess piece you want to _"flick"_ (throw across the board). Make sure it has a collider and rigid body components (if not, add them, as above). Select the `TCastleRigidBody` component of it, and expand the `LinearVelocity` property. Set it to any large non-zero vector, like `-100 0 0`.
