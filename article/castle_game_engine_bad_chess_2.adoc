...
I decided to use `TChessPieceBehavior`, so I added

```delphi
ChessPieceHover, ChessPieceSelected: TChessPieceBehavior;
```

to the `private` section of class `TViewMain`.

The new code in view's `Update` and `Press` methods looks like this:

```delphi
// add CastleColors to the uses clause to define HexToColorRGB utility

TODO
```

You will notice that `MainViewport.TransformUnderMouse` detects what is under the mouse, treating each chess piece as a box. Set `PreciseCollisions` to `true` on all the chess pieces to make the detection more accurate.
